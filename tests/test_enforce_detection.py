...
def test_enabled_enforce_detection_for_non_facial_input():
    ...
    with pytest.raises(ValueError):
        Model.represent(img_path=black_img)

    with pytest.raises(ValueError):
        Model.verify(img1_path=black_img, img2_path=black_img)
...
def test_disabled_enforce_detection_for_non_facial_input_on_represent():
    black_img = np.zeros([224, 224, 3])
    objs = Model.represent(img_path=black_img, enforce_detection=False)
...
def test_disabled_enforce_detection_for_non_facial_input_on_verify():
    black_img = np.zeros([224, 224, 3])
    obj = Model.verify(img1_path=black_img, img2_path=black_img, enforce_detection=False)
