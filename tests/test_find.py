...
def test_find_with_exact_path():
    ...
    dfs = Model.find(img_path=img_path, db_path="dataset", silent=True)
    assert len(dfs) > 0
    for df in dfs:
...
def test_find_with_array_input():
    ...
    dfs = Model.find(img1, db_path="dataset", silent=True)
    assert len(dfs) > 0
    for df in dfs:
        assert isinstance(df, pd.DataFrame)
...
def test_find_with_extracted_faces():
    img_path = os.path.join("dataset", "img1.jpg")
    face_objs = Model.extract_faces(img_path)
    img = face_objs[0]["face"]
    dfs = Model.find(img, db_path="dataset", detector_backend="skip", silent=True)
    assert len(dfs) > 0
    for df in dfs:
        assert isinstance(df, pd.DataFrame)
...
def test_find_without_refresh_database():
    ...
    dfs = Model.find(img_path=img_path, db_path="dataset", silent=True, refresh_database=False)
