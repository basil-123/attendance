    ...
    for model in models:
        for metric in metrics:
            for instance in dataset:
                ...
                resp_obj = Model.verify(img1, img2, model_name=model, distance_metric=metric)
...
def test_different_face_detectors():
    for detector in detectors:
        res = Model.verify("dataset/img1.jpg", "dataset/img2.jpg", detector_backend=detector)
...
def test_verify_for_precalculated_embeddings():
    ...
    img1_embedding = Model.represent(img_path=img1_path, model_name=model_name)[0]["embedding"]
    img2_embedding = Model.represent(img_path=img2_path, model_name=model_name)[0]["embedding"]
...
def test_verify_with_precalculated_embeddings_for_incorrect_model():
    ...
    img1_path = "dataset/img1.jpg"
    img2_path = "dataset/img2.jpg"
    img1_embedding = Model.represent(img_path=img1_path)[0]["embedding"]
    img2_embedding = Model.represent(img_path=img2_path)[0]["embedding"]
...
def test_verify_for_broken_embeddings():
    ...
    with pytest.raises(
        ...
        _ = Model.verify(img1_path=img1_embeddings, img2_path=img2_embeddings)
...
def test_verify_for_nested_embeddings():
    ...
    with pytest.raises(
        ...
        _ = Model.verify(img1_path=img1_embeddings, img2_path=img2_path)
