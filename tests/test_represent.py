...
def test_standard_represent():
    ...
    embedding_objs = Model.represent(img_path)
...
def test_standard_represent_with_io_object():
    img_path = "dataset/img1.jpg"
    default_embedding_objs = Model.represent(img_path)
    io_embedding_objs = Model.represent(open(img_path, 'rb'))
...
def test_represent_for_skipped_detector_backend_with_image_path():
    ...
    img_objs = Model.represent(img_path=face_img, detector_backend="skip")
...
def test_represent_for_skipped_detector_backend_with_preloaded_image():
    ...
    img_objs = Model.represent(img_path=img, detector_backend="skip")
    assert len(img_objs) >= 1
    img_obj = img_objs[0]
    assert "embedding" in img_obj.keys()
    assert "facial_area" in img_obj.keys()
...
def test_max_faces():
    ...
    results = Model.represent(img_path="dataset/couple.jpg")
    assert len(results) > 1

    # test it with max faces arg
    max_faces = 1
    results = Model.represent(img_path="dataset/couple.jpg", max_faces=max_faces)
