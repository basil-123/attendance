...
def test_find_with_exact_path():
    ...
    results = Model.find(img_path=img_path, db_path="dataset", silent=True, batched=True)
...
def test_find_with_array_input():
    ...
    results = Model.find(img1, db_path="dataset", silent=True, batched=True)
    assert len(results) > 0
    for result in results:
        assert isinstance(result, list)
...
def test_find_with_extracted_faces():
    ...
    results = Model.find(img, db_path="dataset", detector_backend="skip", silent=True, batched=True)
    assert len(results) > 0
    for result in results:
        assert isinstance(result, list)
