...
def test_standard_analyze():
    ...
    demography_objs = Model.analyze(img, silent=True)
...
def test_analyze_for_some_actions():
    img = "dataset/img4.jpg"
    demography_objs = Model.analyze(img, ["age", "gender"], silent=True)
...
def test_analyze_for_preloaded_image():
    ...
    resp_objs = Model.analyze(img, silent=True)
